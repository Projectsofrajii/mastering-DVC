...
class DataAnalysis(views.APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response({"message": "format not mached"})

class InstantData(views.APIView):
    def get(self, request):
        try:
            KEY = settings.SECURE_KEY
            if KEY == request.headers['KEY']:
                ...
                for pk in queryset:
                    if pk.instant_records is None or pk.instant_records == "MTRID0052":
                        continue
                    ...
                    if pk.instant_records:
                        ...
                        try:
                            parsed_time = datetime.strptime(instant_time, time_format1)
                        except ValueError:
                            parsed_time = datetime.strptime(instant_time, time_format2)

                        time_difference = time - parsed_time
                        time_difference_str = time_difference.total_seconds()
                        if time_difference_str <= 1800:
                            available_data_100 += 1
                        else:
...
class Profiles(views.APIView):
    ...
    def get(self, request, format=None):
        queryset = RsmMeterMaster.objects.prefetch_related(
            Prefetch('Instant', queryset=InstantModel.objects.order_by('updated_timestamp'),
                     to_attr='instant_records')).all()
...
class Profiled(views.APIView):
    ...
    def get(self, request, format=None):
        ...
        for pk in queryset:
            ...
            if pk.instant_records:
                ins_record = InstantModel.objects.filter(meter_id=pk.meter_id).latest('updated_timestamp')
...
class Profile(views.APIView):#28.62
    ...
    def get(self, request, format=None):
        queryset = RsmMeterMaster.objects.prefetch_related('Instant').all()

        latest_records = []
        for pk in queryset:
            instant_serializer = InstantSeri(pk.Instant.last()) if pk.Instant.exists() else None
...
class Rsm(viewsets.ModelViewSet):
    queryset = InstantModel.objects.all()
    serializer_class = InstantSeri